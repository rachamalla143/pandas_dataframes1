...
logging.basicConfig(filename='mongodb_2.log', level=logging.DEBUG, format = '%(name)s : %(levelname)s : %(asctime)s : %(message)s')
...
#d1=collection.find({"status":{"$gt":"D"}})


#d1=collection.find({"status": {"$gt": "D"}, "item": {"$gt": "C"}})

#collection.update_one({"item":"canvas"},{"$set":{"item":"Nagaraju"}})
#d1 = collection.find({"item":"Nagaraju"})


collection.delete_one({"item" : "Nagaraju"})
d1=collection.find({"item" : "Nagaraju"})
for i in d1:
    logging.info(i)
