...
logging.basicConfig(filename='mongo_con.log', level=logging.DEBUG, format = '%(name)s : %(levelname)s : %(asctime)s : %(message)s')
...
d1 = collection.find({"$and": [{"item": {"$gt": "E"}, "qty": {"$gte": 100}}]})
for i in d1 :
    logging.info(i)
